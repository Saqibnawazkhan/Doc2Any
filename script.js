...
// ========================================
// Stats Animation (Real-time)
// ========================================

function initializeStats() {
    // Get stored values or set defaults
    let fileCount = localStorage.getItem('doc2any_conversions');
    let totalSizeMB = localStorage.getItem('doc2any_total_size');

    if (!fileCount) {
        fileCount = 2128399783;
        localStorage.setItem('doc2any_conversions', fileCount);
    }
    if (!totalSizeMB) {
        totalSizeMB = 39761 * 1024 * 1024; // Convert TB to MB for storage
        localStorage.setItem('doc2any_total_size', totalSizeMB);
    }
    ...
    // Update display
    updateStatsDisplay(fileCount, totalSizeMB);
    ...
    if (totalFilesEl) {
        animateNumber(totalFilesEl, fileCount);
    }
    if (totalSizeEl) {
        const totalTB = Math.round(totalSizeMB / (1024 * 1024));
        animateNumber(totalSizeEl, totalTB);
    }
}

function updateStatsDisplay(fileCount, totalSizeMB) {
    const totalFilesEl = document.getElementById('totalFiles');
    const totalSizeEl = document.getElementById('totalSize');
    ...
    if (totalSizeEl) {
        const totalTB = Math.round(totalSizeMB / (1024 * 1024));
        totalSizeEl.textContent = totalTB.toLocaleString();
    }
}

function incrementConversionCount(fileSizeBytes = 0) {
    // Update file count
    let fileCount = localStorage.getItem('doc2any_conversions') || 2128399783;
    fileCount = parseInt(fileCount) + 1;
    localStorage.setItem('doc2any_conversions', fileCount);

    // Update total size (convert bytes to MB and add)
    let totalSizeMB = localStorage.getItem('doc2any_total_size') || (39761 * 1024 * 1024);
    totalSizeMB = parseFloat(totalSizeMB) + (fileSizeBytes / (1024 * 1024));
    localStorage.setItem('doc2any_total_size', totalSizeMB);
